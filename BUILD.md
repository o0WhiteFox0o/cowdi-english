...
**Các lệnh build production:**

```powershell
# Xóa build cũ + build lại (một lệnh)
Remove-Item -Path .\dist-prod -Recurse -Force; npm run build:prod

# Chỉ build (không xóa trước)
npm run build:prod
```

`build:prod` dùng config vite.config.prod.js với:
- `outDir: 'dist-prod'`
- `base: '/'`
- `VITE_API_URL = 'https://cowdi.net'`
- Vendor chunk tách riêng (`react`, `react-dom`, `react-router-dom`)
